...
for instance, you cannot use '%' string, here document, or '%r' regexp.
...
Otherwise, you can explicitly 'return' the value desired.
...
A stack of values used by the Racc.
...
Racc does have the reserved words but they are only considered as reserved
...
The currently available options are:
  ...
  * [no_]omit_action_call

This is `omit_action_call` by default, which uses the `_reduce_none` action
when the action is empty.
This behavior can be changed to `no_omit_action_call` by specifying the
'--no-omit-actions' command line option of the racc command.
Also, you can set this to `no_omit_action_call` in your grammar file so that
each particular action is defined even when it is empty.
...
Besides, you could not suppress the warning message for reduce/reduce conflict.
...
---- inner
  ruby statement
     ...
     :
--
If four '-' exist on line head, Racc treats this case as the beginning of a
user code block.
