...
module Racc
  ...
  class ParserFileGenerator
    ...
    def state_transition_table
      put_state_transition_table @f   # FIXME
    end
    ...
    def output_reduce_table(out)
      ...
      @grammar.each_with_index do |rl, i|
        ...
        out.printf ' %d, %d, :_reduce_%s',
            ...
            (@params.omit_action_call? and not rl.action) ? 'none' : i.to_s
      end
      out << " ]\n\n"
...
EOS
    ...
    def output_other(out)
      out << "racc_use_result_var = #{@params.result_var?}\n\n"
      out.print(<<EOS)
...
EOS
      ...
      out << @grammar.symbols.map {|tok|
              "'" + tok.to_s.gsub(/'/, '\\\'') + "'" }.join(",\n")
      out << "]\n\n"
      out << "Racc_debug_parser = #{@params.debug_parser?}\n\n"
    end
    ...
    def output_actions(out)
      ...
      result = @params.result_var? ? ', result ' : ''
...
--
      ...
      @grammar.each do |rl|
        act = rl.action
        if not act and @params.omit_action_call?
          out.printf "\n # reduce %d omitted\n",
                     rl.ident
        else
          act ||= defact
      ...
      out.printf <<'--', result, (@params.result_var? ? 'result' : 'val[0]')
